! ニュートン法による解の計算, 解の収束判定をxの振幅で行うnewton とf(x)で行うnewton2 との比較
! 解付近でy方向につぶれたf(x)の場合、newton2 は解の精度が落ちる
! 逆にx方向につぶれたf(x)の場合、newton は解の精度が落ちるが、そもそもf'(x)が限りなく大きくなるため解が求まらない
